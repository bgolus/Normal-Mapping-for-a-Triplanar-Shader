...
// Reconstructed "Bitangents"(?) example shader

// Unlike the Reconstructed Tangents example, the tangent and binormal are not perpendicular to eachother.
// Possibly more accurate? This method is not presented in the original article as I honestly don't know
// what to call it. It is however cheaper than either the derivative cotangents or reconstructed tangents
// shaders. Gets closer to Reoriented Normal Mapping in cost while still providing transform matrices.

Shader "Triplanar/Reconstructed Bitangents"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // half3 bitangentX = normalize(cross(i.worldNormal, half3(0, 0, -axisSign.x)));
                half3 tangentX = normalize(half3(-i.worldNormal.z * axisSign.x, 0.0, i.worldNormal.x * axisSign.x));
                half3 bitangentX = normalize(half3(-i.worldNormal.y * axisSign.x, i.worldNormal.x * axisSign.x, 0.0));
                half3x3 tbnX = half3x3(tangentX, bitangentX, i.worldNormal);

                // half3 tangentY = normalize(cross(i.worldNormal, half3(0, 0, axisSign.y)));
                // half3 bitangentY = normalize(cross(i.worldNormal, half3(-axisSign.y, 0, 0)));
                half3 tangentY = normalize(half3(i.worldNormal.y * axisSign.y, -i.worldNormal.x * axisSign.y, 0.0));
                half3 bitangentY = normalize(half3(0.0, -i.worldNormal.z * axisSign.y, i.worldNormal.y * axisSign.y));
                half3x3 tbnY = half3x3(tangentY, bitangentY, i.worldNormal);

                // half3 tangentZ = normalize(cross(i.worldNormal, half3(0, -axisSign.z, 0)));
                // half3 bitangentZ = normalize(cross(i.worldNormal, half3(axisSign.z, 0, 0)));
                half3 tangentZ = normalize(half3(i.worldNormal.z * axisSign.z, 0.0, -i.worldNormal.x * axisSign.z));
                half3 bitangentZ = normalize(half3(0.0, i.worldNormal.z * axisSign.z, -i.worldNormal.y * axisSign.z));
